...
class TestAddTask:
    def test_add_task_by_category_name(self, db_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        conn = connect(db_path)
        categories = CategoryRepo(conn)
        cat = categories.create("Homelab", COLOR)
        assert main(["--db", str(db_path), "add-task", "--title", "Fix DNS", "--category", "Homelab"]) == 0
    ...
    def test_add_task_by_category_id(self, db_path: Path) -> None:
        ...
        assert main(["--db", str(db_path), "add-task", "--title", "Meeting", "--category", str(cat.id)]) == 0
        tasks = TaskRepo(conn).list_tasks()
        assert tasks[0].category_id == cat.id
        conn.close()
    ...
    def test_add_task_missing_category(self, db_path: Path) -> None:
        with pytest.raises(SystemExit):
            main(["--db", str(db_path), "add-task", "--title", "Foo", "--category", "Nope"])
    ...
    def test_add_task_with_due_date(self, db_path: Path) -> None:
        ...
        main(["--db", str(db_path), "add-task", "--title", "Dentist", "--category", "Personal", "--due", "2026-08-15"])
    ...
    def test_add_task_with_explicit_start(self, db_path: Path) -> None:
        ...
        categories = CategoryRepo(conn)
        categories.create("Personal", COLOR)
        main(["--db", str(db_path), "add-task", "--title", "Trip", "--category", "Personal",
              "--start", "2026-09-01", "--due", "2026-09-10"])
        task = TaskRepo(conn).list_tasks()[0]
...
class TestListTasks:
    ...
    def test_list_filtered_by_status(self, db_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        conn = connect(db_path)
        categories = CategoryRepo(conn)
        cat = categories.create("Work", COLOR)
...
class TestCompleteAndArchive:
    def test_complete_task(self, db_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        conn = connect(db_path)
        categories = CategoryRepo(conn)
        cat = categories.create("Work", COLOR)
    ...
    def test_archive_task(self, db_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        conn = connect(db_path)
        categories = CategoryRepo(conn)
        cat = categories.create("Work", COLOR)
...
class TestDailySummary:
    def test_summary_empty(self, db_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        assert main(["--db", str(db_path), "daily-summary"]) == 0
        assert "Nothing due today" in capsys.readouterr().out

    def test_summary_with_overdue(self, db_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        from datetime import date, timedelta
        conn = connect(db_path)
        categories = CategoryRepo(conn)
        cat = categories.create("Work", COLOR)
        TaskRepo(conn).create(cat.id, "Late thing", due_date=date.today() - timedelta(days=1))
        assert main(["--db", str(db_path), "daily-summary"]) == 0
        captured = capsys.readouterr()
